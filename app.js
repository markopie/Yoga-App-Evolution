 ...
 * ----------------------------------------------------------------------------
 * Core configuration, imports, and global state variables required for the application.
 */
>>>>>>> main
/* ==========================================================================
...
// #endregion
/**
 * SECTION 2: SYSTEM & DOM UTILITIES
 * ----------------------------------------------------------------------------
 * Functions for managing system-level features like screen wake locks.
 */
>>>>>>> main
/* ==========================================================================
...
/**
 * Releases the active screen wake lock if one exists.
 *
 * @returns {Promise<void>}
 */
async function disableWakeLock() {
...
// #endregion
/**
 * SECTION 3: HELPERS & FORMATTING
 * ----------------------------------------------------------------------------
 * Utility functions for string formatting, ID normalization, and data manipulation.
 */
>>>>>>> main
/* ==========================================================================
 ...
 * Converts the Asana Library object into an array for the Browse section.
 *
 * @returns {Array} An array of normalized Asana objects.
 */
function getAsanaIndex() {
  // Safety check if library isn't loaded yet
  if (!asanaLibrary) return [];

  return Object.keys(asanaLibrary)
    .map((id) => {
      // Use the normalizeAsana helper we added earlier
      return normalizeAsana(id, asanaLibrary[id]);
    })
...
=======
/**
 * Normalizes a given ID and resolves it against any known aliases.
 *
 * @param {string|number} id - The raw ID to resolve.
 * @returns {string} The normalized and resolved ID.
 */
function resolveId(id) {
  const norm = normalizePlate(id);
  if (typeof idAliases !== "undefined" && idAliases[norm]) {
...
// #endregion
/**
 * SECTION 4: DATA LOADING & INDEXING
 * ----------------------------------------------------------------------------
 * Functions responsible for fetching, parsing, and caching application data.
 */
>>>>>>> main
/* ==========================================================================
...
// Local Sequence Editing (Save/Reset)
function saveSequencesLocally() {
  if (!sequences || !sequences.length) return;
  if (typeof LOCAL_SEQ_KEY !== "undefined") {
    localStorage.setItem(LOCAL_SEQ_KEY, JSON.stringify(sequences));
...
=======
  ...
  if (manifest && Array.isArray(manifest.images)) {
    manifest.images.forEach((filename) => {
      ...
      // Normalize
      const cleanId =
        typeof normalizePlate === "function" ? normalizePlate(rawID) : rawID;
...
=======
  ...
  return (
    asanaArray.find((a) => {
      const cleanLibId = String(a.id || a.asanaNo || "")
        .trim()
        .replace(/^0+/, "");
      return cleanLibId === cleanSearchId;
    }) || null
...
// #endregion
/**
 * SECTION 5: HISTORY & LOGGING
 * ----------------------------------------------------------------------------
 * Mechanisms for tracking user progress, saving to local storage, and syncing with the server.
 */
>>>>>>> main
/* ==========================================================================
...
=======
/**
 * Calculates the effective duration for a pose, accounting for sides if necessary.
 *
 * @param {string|Array} id - The identifier for the Asana.
 * @param {number|string} dur - The base duration in seconds.
 * @returns {number} The total effective time in seconds.
 */
function getEffectiveTime(id, dur) {
  // Unwrap the ID if it's hiding in an array (fixes Player math)
  let rawId = id;
  if (Array.isArray(rawId)) rawId = rawId[0];
  if (Array.isArray(rawId)) rawId = rawId[0]; // Double unwrap just in case
  ...
  // The ID Fix: Compare them mathematically (so "003" safely matches "3")
  const searchId = Number(rawId);
  const asana = Object.values(lib).find(
    (a) => Number(a.id || a.asanaNo) === searchId,
  );

  const duration = Number(dur) || 0;

  // Double the time if the pose requires sides
  if (asana && (asana.requiresSides || asana.requires_sides)) {
    return duration * 2;
  }
...
function lastCompletionFor(title) {
  const log = loadCompletionLog().filter(
    (x) => x && x.title === title && typeof x.ts === "number",
  );
  if (!log.length) return null;
  return log.sort((a, b) => b.ts - a.ts)[0];
}
...
function seedManualCompletionsOnce() {
  const log = loadCompletionLog();
  const have = new Set(
    log.filter((x) => x?.title).map((x) => x.title + "::" + x.ts),
  );
  const seeds = [
    {
...
/**
 * Fetches the user's sequence completion history from the server (Supabase).
 * Falls back to local storage if the database is unavailable.
 *
 * @returns {Promise<Array>} An array of completion record objects.
 */
async function fetchServerHistory() {
  try {
    if (!supabase) {
...
/**
 * Appends a new sequence completion record to the server and local cache.
 *
 * @param {string} title - The title of the completed sequence.
 * @param {Date} whenDate - The date and time of completion.
 * @param {string|null} [category=null] - The category of the sequence.
...
=======
  if (!isoStrings || !isoStrings.length) return 0;
  const days = [
    ...new Set(isoStrings.map((s) => new Date(s).toLocaleDateString("en-AU"))),
  ]
    .map((d) => {
      const [dd, mm, yyyy] = d.split("/");
      return new Date(yyyy, mm - 1, dd).getTime();
...
=======
  ...
  if (seq && seq.poses) {
    const poses =
      typeof getExpandedPoses === "function"
        ? getExpandedPoses(seq)
        : seq.poses;
    if (poses[state.poseIdx]) {
      const rawId = Array.isArray(poses[state.poseIdx][0])
        ? poses[state.poseIdx][0][0]
        : poses[state.poseIdx][0];
      const asana =
        typeof findAsanaByIdOrPlate === "function"
          ? findAsanaByIdOrPlate(normalizePlate(rawId))
          : null;
      if (asana) {
        poseName =
...
// #endregion
/**
 * SECTION 6: CORE PLAYER LOGIC
 * ----------------------------------------------------------------------------
 * Initialization, timer orchestration, navigation, and rendering of active sequences.
 */
/* ==========================================================================
   APP INITIALIZATION (Controller)
   ========================================================================== */
...
async function init() {
  ...
  try {
    ...
    // Core Config
    if (typeof seedManualCompletionsOnce === "function")
      seedManualCompletionsOnce();

    // Load Overrides + History in Parallel
    await Promise.all([
      typeof loadManifestAndPopulateLists === "function"
        ? loadManifestAndPopulateLists()
        : Promise.resolve(),
      fetchServerHistory(),
    ]);

    // Load Main Data (Sequential)
    if (statusEl) statusEl.textContent = "Loading library...";
    asanaLibrary = await loadAsanaLibrary();
    window.asanaLibrary = asanaLibrary;
...
/**
 * Expands a sequence by unpacking any macro-references into their constituent poses.
 *
 * @param {Object} sequence - The sequence object containing a poses array.
 * @returns {Array} A flat array of unpacked pose data.
 */
function getExpandedPoses(sequence) {
  let expanded = [];
  if (!sequence || !sequence.poses) return [];
...
playbackEngine.onStart = () => {
  ...
  try {
    ...
    if (poses[currentIndex]) {
      const rawId = Array.isArray(poses[currentIndex][0])
        ? poses[currentIndex][0][0]
        : poses[currentIndex][0];
      const asana =
        typeof findAsanaByIdOrPlate === "function"
          ? findAsanaByIdOrPlate(normalizePlate(rawId))
          : null;
...
playbackEngine.onTransitionStart = (secs) => {
  ...
  const poses =
    activePlaybackList && activePlaybackList.length > 0
      ? activePlaybackList
      : currentSequence?.poses || [];
  let previewName = "";
  const nextIdx = currentIndex + 1;
  ...
  if (nextIdx < poses.length) {
    ...
    const asana =
      typeof findAsanaByIdOrPlate === "function"
        ? findAsanaByIdOrPlate(normalizePlate(id))
        : null;
    previewName = asana
      ? typeof displayName === "function"
        ? displayName(asana)
  ...
  if (nextPoseEl)
    nextPoseEl.textContent = previewName ? `Next: ${previewName}` : "";
...
function updateTimerUI(remaining, currentPoseSeconds) {
  ...
  if (currentSequence) {
    ...
    const totalSeconds = poses.reduce(
      (acc, p) => acc + getEffectiveTime(p[0], p[1]),
      0,
    );
    let secondsLeft = remaining;

    if (
      typeof needsSecondSide !== "undefined" &&
      needsSecondSide &&
      poses[currentIndex]
    ) {
      secondsLeft += Number(poses[currentIndex][1]) || 0;
    ...
    if (remDisp && typeof formatHMS === "function")
      remDisp.textContent = formatHMS(secondsLeft);
    if (totDisp && typeof formatHMS === "function")
      totDisp.textContent = formatHMS(totalSeconds);
...
=======
  ...
  // Unpack macros first
  const expanded =
    typeof getExpandedPoses === "function" ? getExpandedPoses(seq) : seq.poses;
...
=======
function nextPose() {
  // Get the correct list (Always prefer the expanded playback list)
  const poses =
    window.activePlaybackList && window.activePlaybackList.length > 0
      ? window.activePlaybackList
      : currentSequence.poses || [];
  ...
  // Scenario: Advance to next index in the 87-item list
  if (currentIndex < poses.length - 1) {
    currentSide = "right";
    needsSecondSide = false;
    setPose(currentIndex + 1);
...
function prevPose() {
  ...
  if (currentIndex > 0) {
    ...
    // Use our helper to check if the previous pose in the list has sides
    const id = Array.isArray(prevPoseData[0])
      ? prevPoseData[0][0]
      : prevPoseData[0];
    const asana = findAsanaByIdOrPlate(normalizePlate(id));
...
/* ==========================================================================
   ...
   ========================================================================== */
/**
 * Renders the UI and updates the playback engine for a specific pose index.
 *
 * @param {number} idx - The index of the pose within the active sequence.
 * @param {boolean} [keepSamePose=false] - Whether to retain the current side/variation state.
 * @returns {void}
 */
function setPose(idx, keepSamePose = false) {
  if (!currentSequence) return;
  ...
  // SAVE PROGRESS (update currentIndex first so the correct pose index is saved)
  currentIndex = idx;
  if (typeof saveCurrentProgress === "function") saveCurrentProgress();
  ...
  const originalRowIndex =
    currentPose && currentPose[5] !== undefined ? currentPose[5] : idx;

  // FIX: Use the original sequence length instead of the expanded 'poses.length'
  const displayTotal = currentSequence.poses
  ...
  if (
    ...
    if (varHoldStr) {
      ...
      if (varHd.standard > 0) {
        ...
        // MATCH: Use .short and .long here too
        const min =
          varHd.short || Math.max(5, Math.round(varHd.standard * 0.5));
        const std = varHd.standard;
        const max = varHd.long || Math.round(varHd.standard * 2.0);

        if (val < 50) seconds = Math.round(min + (std - min) * (val / 50));
        else if (val > 50)
          seconds = Math.round(std + (max - std) * ((val - 50) / 50));
        else seconds = std;
      }
    }
  }
  ...
  if (dial) {
    ...
    if (val !== 50) {
      let mult = 1.0;
      if (val < 50)
        mult = 0.5 + (val / 50) * 0.5; // Faster (0.5x to 1x)
      else mult = 1.0 + ((val - 50) / 50) * 1.0; // Slower (1x to 2x)
      seconds = Math.round(seconds * mult);
    }
  }
  ...
  // --- NEW: VARIATION & NOTE EXTRACTION ---
  // Combine all extra array fields to cleanly parse brackets vs actual notes
  let rawExtras = [currentPose[2], currentPose[3], currentPose[4]]
    .filter(Boolean)
    .join(" ")
    .trim();
  let variationTitle = "";
  ...
  if (bracketMatch) {
    ...
    // Legacy fallback: If no brackets exist, currentPose[2] acts as a base name override
    baseOverrideName = currentPose[2] || "";
    actualNote = [currentPose[3], currentPose[4]]
      .filter(Boolean)
      .join(" ")
      .trim();
  }
  ...
  if (asana && asana.variations && variationTitle) {
    ...
    for (const [vKey, vData] of Object.entries(asana.variations)) {
      const resolvedTitle =
        typeof vData === "object" ? vData.title || vData.Title || "" : "";
      const normTitle = normalizeText(resolvedTitle);
      const normShort = normalizeText(
        typeof vData === "object"
          ? vData.shorthand || vData.Shorthand || ""
          : "",
      );
      const normKey = vKey.toLowerCase();

      if (
        normVarTitle === normTitle ||
        normVarTitle === normShort ||
        normVarTitle === `stage ${normKey}` ||
        normVarTitle === normKey
      ) {
        const varTech =
          typeof vData === "object"
            ? vData.Full_Technique || vData.technique
            : vData;
        if (varTech) displayTechnique = varTech;
        if (typeof vData === "object")
          displayShorthand = vData.shorthand || vData.Shorthand || "";
        if (resolvedTitle) variationTitle = resolvedTitle;
  ...
  // Legacy fallback if the old index 3 was used
    ...
    currentPose[3] &&
    asana.variations &&
    asana.variations[currentPose[3]]
  ) {
    const v = asana.variations[currentPose[3]];
    if (typeof v === "string") {
      displayTechnique = v;
  ...
  if (textContainer) {
    if (displayTechnique && typeof formatTechniqueText === "function") {
      ...
      if (variationTitle) {
        techniqueHTML =
          `<div style="font-weight:600; color:#333; margin-bottom:8px; padding-bottom:5px; border-bottom:1px solid #ddd;">${variationTitle} Instructions:</div>` +
          techniqueHTML;
      }
...
=======
  ...
  // NOTES UI (Passes only the clean actualNote)
  if (typeof updatePoseNote === "function") updatePoseNote(actualNote);
  if (typeof updatePoseAsanaDescription === "function")
    updatePoseAsanaDescription(asana);
  if (typeof loadUserPersonalNote === "function")
    loadUserPersonalNote(lookupId);

  // META UI & AUDIO BUTTON
  const metaContainer = document.getElementById("poseMeta");
  if (metaContainer) {
    metaContainer.innerHTML = "";
...
function updatePoseAsanaDescription(asana) {
  ...
  const text = (asana?.description || asana?.Description || "")
    .toString()
    .trim();
  if (!text) {
    details.style.display = "none";
...
function updateTotalAndLastUI() {
  // EXPLORER FIX: Look at the activePlaybackList to include the injected standing poses
  const poses =
    window.activePlaybackList && window.activePlaybackList.length > 0
      ? window.activePlaybackList
      : currentSequence && currentSequence.poses
        ? currentSequence.poses
        : [];
  ...
  if (lastEl) {
    const title =
      currentSequence && currentSequence.title ? currentSequence.title : null;

    if (title) {
      const source =
        typeof serverHistoryCache !== "undefined" &&
        Array.isArray(serverHistoryCache) &&
        serverHistoryCache.length
          ? serverHistoryCache
          : typeof loadCompletionLog === "function"
            ? loadCompletionLog()
            : [];
...
function loadUserPersonalNote(idField) {
  ...
  area.style.fontFamily = "inherit";
  area.placeholder =
    "Add your personal notes for this pose here (e.g. 'Use block under knee')...";
  area.value = savedNote;
...
// #endregion
/**
 * SECTION 7: BROWSING UI CONFIGURATION
 * ----------------------------------------------------------------------------
 * Setup and orchestration for the primary browsing interface.
 */
>>>>>>> main
/* ==========================================================================
...
=======
function setupBrowseUI() {
  // Wire up the main Browse button
  const bBtn = document.getElementById("browseBtn");
  if (bBtn) {
    bBtn.onclick = (e) => {
      e.preventDefault();
  ...
  // Hide Finals Checkbox
  const finalsChk = $("browseFinalOnly");
  if (finalsChk) {
    if (
      finalsChk.parentElement &&
      finalsChk.parentElement.tagName === "LABEL"
    ) {
      finalsChk.parentElement.style.display = "none";
    } else {
      finalsChk.style.display = "none";
  ...
  if (closeBtn && !document.getElementById("browseAddAsanaBtn")) {
    ...
    addBtn.style.cssText =
      "background: #007aff; color: white; margin-right: 8px;";
...
function renderBrowseList(items) {
  ...
  items.slice(0, 400).forEach((asma) => {
    ...
    // Fallback logic for title
    let titleText =
      typeof displayName === "function" ? displayName(asma) : null;
    if (!titleText || titleText === "(no name)") {
      titleText = asma.name || asma.english || asma.iast || "(no name)";
    }
    ...
    const meta = document.createElement("div");
    meta.className = "meta";
    const catDisplay = asma.category
      ? asma.category.replace(/^\d+_/, "").replace(/_/g, " ")
      : "Uncategorized";
    const catBadge = catDisplay
      ? ` <span class="badge">${catDisplay}</span>`
      : "";
    ...
    if (typeof asma.plates === "object" && asma.plates !== null) {
      const finalStr =
        asma.plates.final && asma.plates.final.length
          ? `Final: ${asma.plates.final.join(", ")}`
          : "";
      const interStr =
        asma.plates.intermediate && asma.plates.intermediate.length
          ? `Int: ${asma.plates.intermediate.join(", ")}`
          : "";
      platesText = [finalStr, interStr].filter(Boolean).join(" | ");
    } else {
      platesText = asma.plates || asma.plate_numbers || "";
...
=======
    ...
    btn.addEventListener("click", () => {
      if (typeof showAsanaDetail === "function") showAsanaDetail(asma);
      if (typeof isBrowseMobile === "function" && isBrowseMobile()) {
        if (typeof enterBrowseDetailMode === "function")
          enterBrowseDetailMode();
      }
    });
...
=======
  const plates =
    asma.finalPlates && asma.finalPlates.length
      ? asma.finalPlates
      : asma.interPlates;
  if (!plates || !plates.length) return;
  ...
  const variationName = asma.variation || "";
  const fullName = variationName
    ? `${asma.english} (${variationName})`
    : asma.english;
...
async function showAsanaDetail(asana) {
  ...
  editBtn.style.cssText =
    "background: #2196f3; color: white; padding: 6px 12px; cursor: pointer; margin-bottom: 10px; font-weight: bold; border: none; border-radius: 6px;";
  editBtn.onclick = () => {
    window.openAsanaEditor(asana.id || asana.asanaNo);
  };
  d.appendChild(editBtn);
  ...
  // Build the rest of the Info via a single HTML string
  // Use a unique name for this string variable to avoid re-declaration errors
  let detailHTML = `
>>>>>>> main
      ${
...
=======
  // Append Images
  const urls =
    typeof smartUrlsForPoseId === "function"
      ? smartUrlsForPoseId(asana.id || asana.asanaNo)
      : [];
  if (urls && urls.length > 0) {
    detailHTML += `<div class="browse-collage">`;
    urls.forEach((src) => {
  ...
  // Append Technique (Base Pose)
  const baseTech = asana.technique || asana.Technique || "";
  if (baseTech) {
    detailHTML += `<h3>Base Technique</h3>
          <div class="technique-text" style="white-space: pre-wrap;">${
            typeof formatTechniqueText === "function"
              ? formatTechniqueText(baseTech)
              : baseTech
          }</div>`;
  }
  ...
  if (asana.variations && Object.keys(asana.variations).length > 0) {
    ...
    sortedKeys.forEach((key) => {
      ...
      if (typeof val === "string") {
        ...
        techText =
          val.full_technique || val.Full_Technique || val.technique || "";
        shortText = val.shorthand || val.Shorthand || "";
        const actualTitle =
          val.Title || val.title || val.Stage_Title || val.stage_title;
      ...
      if (shortText)
        html += `<div style="color:${isCustom ? "#1565c0" : "#2e7d32"}; font-weight:bold; margin-bottom:8px; font-family:monospace; font-size:1rem;">${shortText}</div>`;

      // Add the Hold time if it exists
      if (holdText)
        html += `<div style="color:${isCustom ? "#0d47a1" : "#666"}; margin-bottom:8px; font-weight:600; font-size:0.95rem;">Hold: ${holdText}</div>`;

      if (techText) {
        const formattedTech =
          typeof formatTechniqueText === "function"
            ? formatTechniqueText(techText)
            : techText;
        html += `<div class="technique-text" style="white-space: pre-wrap; font-size:0.95rem; color:#444;">${formattedTech}</div>`;
      } else {
        html += `<div class="muted" style="font-size:0.85rem;">No specific instructions provided.</div>`;
...
function renderPlateSection(title, plates, globalSeen, fallbackId) {
  ...
  const processIds = (idList) => {
    for (const p of idList) {
      if (!p || p === "undefined") continue;
      const u =
        typeof urlsForPlateToken === "function" ? urlsForPlateToken(p) : [];
      if (!u.length) missing.push(p);
      u.forEach((x) => {
        const g = globalSeen || null;
        if (!seen.has(x) && !(g && g.has(x))) {
  ...
  if (urls.length === 0 && fallbackId) {
    const fallbackUrls =
      typeof urlsForPlateToken === "function"
        ? urlsForPlateToken(fallbackId)
        : [];
    if (fallbackUrls.length > 0) {
      fallbackUrls.forEach((x) => {
        const g = globalSeen || null;
...
function renderCollage(urls) {
  ...
  urls.forEach((u) => {
    const mob =
      typeof mobileVariantUrl === "function" ? mobileVariantUrl(u) : u;
    const tile = document.createElement("div");
    tile.className = "tile";
    tile.innerHTML = `
>>>>>>> main
...
function applyBrowseFilters() {
  ...
  const cat = document.getElementById("browseCategory")?.value || "";
  const finalsOnly =
    document.getElementById("browseFinalOnly")?.checked || false;
  ...
  const filtered = asanaArray.filter((a) => {
    ...
    if (plateStr) {
      const plateArr = plateStr.match(/\d+/g) || [];
      const aPlates =
        String(a.plates || a.plate_numbers || "").match(/\d+/g) || [];
      const hasPlate = plateArr.some((p) => aPlates.includes(p));
      if (!hasPlate) return false;
    }
  ...
  const seen = new Set();
  filtered.forEach((a) => {
    const uniqueKey = String(a.id || a.asanaNo || a.name || "")
      .toLowerCase()
      .trim();
    if (uniqueKey && !seen.has(uniqueKey)) {
      seen.add(uniqueKey);
...
// #endregion
/**
 * SECTION 8: SEQUENCE BUILDER & DATA LAYER
 * ----------------------------------------------------------------------------
 * Functions supporting the sequence builder tool and associated data persistence.
 */
...
function builderRender() {
  ...
  builderPoses.forEach((pose, idx) => {
    ...
    if (isMacro) {
      const targetTitle = idStr.replace("MACRO:", "").trim();
      const subCourse = window.courses
        ? window.courses.find((c) => c.title === targetTitle)
        : null;

      if (subCourse && subCourse.poses) {
        const oneRoundSecs = subCourse.poses.reduce(
          (acc, sp) => acc + getEffectiveTime(sp[0], sp[1]),
          0,
        );
        totalSec += oneRoundSecs * durOrReps;
      }
      ...
      const normId =
        typeof normalizePlate === "function" ? normalizePlate(idStr) : idStr;
      asana = libraryArray.find(
        (a) => String(a.id || a.asanaNo) === String(normId),
      );

      // IF NOT FLOW: Force library standard timing
      const libraryStd =
        asana && asana.hold_data ? asana.hold_data.standard : 30;
      const activeTime = isFlow ? durOrReps : libraryStd;
    ...
    // BUILD THE DURATION INPUT (Locked if not Flow/Macro)
    const displayTime = isMacro
      ? durOrReps
      : isFlow
        ? durOrReps
        : asana?.hold_data?.standard || 30;
    const isLocked = !isFlow && !isMacro;
...
=======
  ...
  qS(".b-id").forEach(
    (el) =>
      (el.onchange = (e) => {
        ...
        builderPoses[i].id = val;
        const normId =
          typeof normalizePlate === "function" ? normalizePlate(val) : val;
        const asanaMatch = libraryArray.find(
          (a) => String(a.id || a.asanaNo) === String(normId),
        );
        if (asanaMatch) {
          builderPoses[i].name = asanaMatch.name;
          if (asanaMatch.hold_data?.standard)
            builderPoses[i].duration = asanaMatch.hold_data.standard;
        }
        builderRender();
      }),
  );

  qS(".b-std-time").forEach(
    (el) =>
      (el.onclick = () => {
        const i = el.dataset.idx;
        const normId =
          typeof normalizePlate === "function"
            ? normalizePlate(builderPoses[i].id)
            : builderPoses[i].id;
        const asanaMatch = libraryArray.find(
          (a) => String(a.id || a.asanaNo) === String(normId),
        );
  ...
  qS(".b-note").forEach(
    (el) =>
      (el.oninput = (e) =>
        (builderPoses[e.target.dataset.idx].note = e.target.value)),
  );
  qS(".b-move-up").forEach(
    (el) => (el.onclick = () => movePose(parseInt(el.dataset.idx), -1)),
  );
  qS(".b-move-dn").forEach(
    (el) => (el.onclick = () => movePose(parseInt(el.dataset.idx), 1)),
  );
  qS(".b-remove").forEach(
    (el) => (el.onclick = () => removePose(parseInt(el.dataset.idx))),
  );

  qS(".b-move-top").forEach(
    (el) =>
      (el.onclick = () => {
        const idx = parseInt(el.dataset.idx);
        if (idx > 0) {
  ...
  if (statsEl) {
    ...
    builderPoses.forEach((p) => {
      ...
      if (idStr.startsWith("MACRO:")) {
        ...
        const sub = (window.courses || []).find((c) => c.title === targetTitle);
        if (sub && sub.poses) {
          const oneRound = sub.poses.reduce(
            (acc, sp) => acc + getEffectiveTime(sp[0], sp[1]),
            0,
          );
          finalTotalSecs += oneRound * durOrReps;
...
// HELPER FUNCTION: Handles the actual Supabase injection
async function processSemicolonCommand(commandString) {
  const parts = commandString.split(";").map((p) => p.trim());
  if (parts.length < 3) return;
...
=======
  ...
  const exists = courses.find(
    (c) => c.title.trim().toLowerCase() === targetTitle.trim().toLowerCase(),
  );
  if (!exists) {
    alert("Sequence not found. Please enter the exact title.");
    return;
...
async function fetchAudioOverrides() {
    try {
        const data = await loadJSON(AUDIO_OVERRIDE_URL, {});
        if (data) audioOverrides = data;
...
// #endregion
/**
 * SECTION 9: WIRING UP UI ELEMENTS
 * ----------------------------------------------------------------------------
 * Event listeners and DOM bindings that connect the user interface to application logic.
 */
>>>>>>> main
/* ==========================================================================
...
if (seqSelect) {
  ...
  if (!document.getElementById("quickEditBtn")) {
    ...
    editBtn.className = "tiny";
    editBtn.style.cssText =
      "margin-left: 8px; padding: 4px 10px; font-size: 1.1rem; vertical-align: middle;";
    seqSelect.parentNode.insertBefore(editBtn, seqSelect.nextSibling);
    editBtn.onclick = () => {
      if (!currentSequence) return alert("Select a sequence first.");
      openEditCourse();
    ...
    newBtn.style.cssText =
      "margin-left: 4px; padding: 4px 10px; vertical-align: middle;";
    editBtn.parentNode.insertBefore(newBtn, editBtn.nextSibling);
    newBtn.onclick = () => builderOpen("new", null);
  }
...
=======
    const rawSequence = (typeof courses !== "undefined" ? courses : sequences)[
      parseInt(idx, 10)
    ];
    currentSequence = rawSequence;
    ...
    if (typeof getExpandedPoses === "function") {
      ...
      activePlaybackList = currentSequence.poses
        ? [...currentSequence.poses]
        : [];
    }
...
=======
  ...
  function updateBtn() {
    ...
    btn.style.opacity = using ? "1" : "0.7";
    btn.title = using
      ? "Showing IAST names — click for English"
      : "Showing English names — click for IAST";
  }
  updateBtn();
  btn.addEventListener("click", () => {
    setIASTPref(!prefersIAST());
...
function updateLastCompletedPill() {
  ...
  if (last) {
    pill.textContent = "Last: " + new Date(last).toLocaleString();
  } else {
    pill.textContent = "History";
  }
}

// History Interactions (Clickable Pill)
const lastPill = $("lastCompletedPill");
if (lastPill) {
  lastPill.style.cursor = "pointer";
  lastPill.title = "Click to view full completion history";
...
if ($("clearHistoryBtn"))
  $("clearHistoryBtn").onclick = async () => {
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = "Clear This Sequence";
    }
    openHistoryModal("current");
    updateTotalAndLastUI();
  };
    ...
    if (entries.length === 0) {
      ...
      if (streak > 0) {
        ...
        streakEl.style.cssText =
          "padding:8px 10px; background:#e8f5e9; color:#2e7d32; font-weight:bold; border-radius:6px; margin-bottom:8px; font-size:0.9rem;";
      ...
      entries.forEach((e) => {
        const row = document.createElement("div");
        row.style.cssText =
          "padding:8px 4px; border-bottom:1px solid #f0f0f0; display:flex; justify-content:space-between; align-items:center; font-size:0.9rem;";
        const d = new Date(e.ts);
        ...
        delBtn.style.cssText =
          "color:#999; border:none; background:transparent; cursor:pointer; font-size:0.8rem;";
        delBtn.title = "Remove this entry";
        delBtn.onclick = async () => {
          delBtn.disabled = true;
          await deleteCompletionById(e.id);
...
=======
  ...
  entries.forEach((e) => {
    if (!e.title) return;
    if (!byTitle[e.title])
      byTitle[e.title] = { category: e.category || "", isos: [], lastTs: 0 };
    byTitle[e.title].isos.push(e.iso);
    if (e.ts > byTitle[e.title].lastTs) byTitle[e.title].lastTs = e.ts;
  });
  ...
  statsHeader.style.cssText =
    "padding:10px 14px; background:#e8f5e9; border-radius:6px; margin-bottom:12px; font-size:0.9rem;";
  let statsHtml = `<div style="font-weight:bold; font-size:1rem; margin-bottom:4px;">Total sessions: ${totalCompletions}</div>`;
  if (overallStreak > 1) {
    statsHtml += `<div style="color:#2e7d32; font-weight:bold;">${overallStreak}-day practice streak — keep it up!</div>`;
  } else if (overallStreak === 1) {
  ...
  Object.keys(byTitle).forEach((title) => {
    const cat =
      byTitle[title].category || titleToCat[title] || "Archived / Removed";
    if (!grouped[cat]) grouped[cat] = [];
    grouped[cat].push({
      title,
      count: byTitle[title].isos.length,
...
=======
      section.style.cssText =
        "margin-bottom:10px; border:1px solid #ddd; border-radius:6px; background:#fff;";

      const summary = document.createElement("summary");
      summary.style.cssText =
        "padding:10px; cursor:pointer; font-weight:bold; background:#f5f5f5; border-radius:6px 6px 0 0;";
>>>>>>> main
...
=======
      items.forEach((item) => {
        const row = document.createElement("div");
        row.style.cssText =
          "display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid #eee; font-size:0.9rem;";
        let countColor = "#eee";
        if (item.count > 5) countColor = "#ffe0b2";
        if (item.count > 10) countColor = "#c8e6c9";
        row.innerHTML = `
...
safeListen("resetBtn", "click", () => {
  ...
  // WIPE MEMORY (Fixes the "Resume" popup on refresh)
  localStorage.removeItem("lastPlayedSequence");
  localStorage.removeItem("currentPoseIndex");
  localStorage.removeItem("timeLeft");
  ...
  // RESET DROPDOWN (Fixes the ID: sequenceSelect)
  const dropdown = $("sequenceSelect");
  if (dropdown) dropdown.value = "";
  ...
  // RESET UI VISUALS (Matched to your specific HTML IDs)
  const titleEl = $("poseName");
  const metaEl = $("poseMeta"); // Used for english/sanskrit subtext
  const collageEl = $("collageWrap"); // Where images go
  const timerEl = $("poseTimer");
...
function updateDialUI() {
  ...
  if (estEl && currentSequence && window.currentSequenceOriginalPoses) {
    const total = window.currentSequenceOriginalPoses.reduce((s, p) => {
      const origDur = Number(p[1]) || 0;
      const id = Array.isArray(p[0]) ? p[0][0] : p[0];
      const asana = findAsanaByIdOrPlate
        ? findAsanaByIdOrPlate(normalizePlate(id))
        : null;
      const { short, defaultDur, long } = resolveDialAnchors(origDur, asana);
      const dur = interpolateDuration(pos, short, defaultDur, long);
...
safeListen("historyLink", "click", (e) => {
    ...
    toggleHistoryPanel();
=======
  // Sliding: Updates math and UI dynamically
  durationDial.addEventListener("input", () => {
    let val = parseInt(durationDial.value, 10);
    // If the finger is between 45 and 55, force it to 50
    if (val > 45 && val < 55) {
  ...
  // Releasing: Just forces one final sync
  durationDial.addEventListener("change", () => {
    if (currentSequence) applyDurationDial();
  });

  // Double Click: Reset (Keep for desktop users)
  durationDial.addEventListener("dblclick", () => {
    durationDial.value = 50;
    if (typeof updateDialUI === "function") updateDialUI();
    if (currentSequence) applyDurationDial();
...
function applyDurationDial() {
  ...
  // Expand a fresh list from the raw sequence
  const baseList =
    typeof getExpandedPoses === "function"
      ? getExpandedPoses(currentSequence)
      : currentSequence.poses;

  // Apply the dynamic database scale
  window.activePlaybackList = baseList.map((p) => {
    let cloned = [...p];
    const rawId = Array.isArray(p[0]) ? p[0][0] : p[0];
    ...
    if (
      asana &&
      asana.hold_json &&
      typeof asana.hold_json.standard === "number"
    ) {
      const hj = asana.hold_json;
      ...
      const originalSeconds = Number(cloned[1]) || 30;
      let mult =
        val < 50 ? 0.5 + (val / 50) * 0.5 : 1.0 + ((val - 50) / 50) * 1.0;
      cloned[1] = Math.round(originalSeconds * mult);
    }
    return cloned;
  });
  ...
  // Update the Active Timer (Mid-pose scaling)
  if (window.activePlaybackList[currentIndex]) {
    const newPoseSeconds =
      Number(window.activePlaybackList[currentIndex][1]) || 0;
...
function builderOpen(mode, seq) {
  ...
  if (catInput && datalist) {
    ...
    datalist.innerHTML = allCats
      .map((c) => `<option value="${c}"></option>`)
      .join("");

    let tempVal = "";
    catInput.onfocus = () => {
      tempVal = catInput.value;
      catInput.value = "";
    };
    catInput.onblur = () => {
      if (catInput.value === "") catInput.value = tempVal;
    };
  }
  ...
  if (mode === "new") {
    ...
    const libraryArray = Object.values(asanaLibrary || {});
    const rawPoses =
      window.currentSequenceOriginalPoses && seq === currentSequence
        ? window.currentSequenceOriginalPoses
        : seq.poses || [];

    rawPoses.forEach((p) => {
      const id = String(Array.isArray(p[0]) ? p[0][0] : p[0] || "").padStart(
        3,
        "0",
      );
      const asana = libraryArray.find((a) => String(a.id) === id);
      ...
      if (!variation && asana?.variations && extractedLabel) {
        ...
        for (const vKey of sortedKeys) {
          ...
          const vTitle = (vData?.title || "").toLowerCase();
          if (
            extractedLabel.toLowerCase() === vTitle ||
  ...
  $("editCourseBackdrop").style.display = "flex";
  setTimeout(() => {
    if ($("builderSearch")) $("builderSearch").focus();
  }, 50);
}
...
async function builderSave() {
  ...
  const totalSec = builderPoses.reduce((acc, p) => {
    ...
    if (idStr.startsWith("MACRO:")) {
      ...
      if (sub && sub.poses) {
        const oneRound = sub.poses.reduce(
          (accSub, sp) => accSub + getEffectiveTime(sp[0], sp[1]),
          0,
        );
        return acc + oneRound * durOrReps;
      }
...
=======
  ...
  if (datalist && window.courses) {
    // Grab all unique sequence titles
    const allTitles = [
      ...new Set(window.courses.map((c) => c.title).filter(Boolean)),
    ].sort();
    datalist.innerHTML = allTitles
...
=======
  const targetTitle = document.getElementById("linkSequenceInput").value.trim();
  const reps =
    parseInt(document.getElementById("linkSequenceReps").value, 10) || 1;
...
(function setupBuilderSearch() {
  ...
  input.addEventListener("input", () => {
      ...
      completedIds.forEach((id) => {
        ...
        if (asana) {
          builderPoses.push({
            ...
            name: displayName(asana),
            duration:
              asana.hold_data && asana.hold_data.standard
                ? asana.hold_data.standard
                : 30,
            note: "",
          });
        }
    ...
    debounceTimer = setTimeout(() => {
      ...
      // Smart ID Padding: If user types "1", we also look for "001"
      let paddedQ = "";
      if (/^\d+$/.test(q)) {
        paddedQ = q.padStart(3, "0");
      }
...
=======
    ...
    if (e.key === "Enter" && !e.shiftKey) {
      ...
      if (trimmedVal.includes(";")) {
        ...
        if (lines.length > 1) {
          if (
            confirm(
              `Batch Mode: Detected ${lines.length} sequences. Create all now?`,
            )
          ) {
            try {
              for (const line of lines) {
  ...
  results.addEventListener("click", (e) => {
    ...
    const defaultDuration =
      asana && asana.hold_data && asana.hold_data.standard
        ? asana.hold_data.standard
        : 30;
  ...
  if (blankBtn) {
    blankBtn.addEventListener("click", () => {
      ...
      // THIS IS THE MISSING PIECE: Force the UI to update
      builderRender();

      // Optional: Auto-focus the ID input of the new top row
      setTimeout(() => {
        const firstIdInput = document.querySelector(".b-id");
        if (firstIdInput) firstIdInput.focus();
      }, 50);
